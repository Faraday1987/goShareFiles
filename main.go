...
Copyright (c) 2020-2024 Faraday <faraday1987@gmail.com>
...
func main() {
	...
	// Get the IP adress where it publish the server
	host, _ := os.Hostname()
	local, _ := net.LookupIP(host)
	...
	conn, err := net.Dial("udp", "8.8.8.8:80")   //create google connection
	defer conn.Close()                           //close conection
	...
	bind := flag.String("bind", ":7777", "Bind address")
	...
	fmt.Printf("Sharing 📂%s on -> 👻 %s or [%s] Port%s\n", *root_folder, local, localAddr.IP, *bind)
